...
#videofeeling is a short term container for average predicted emotions per 6 second emotional expression
videofeeling = []
#total is the long term container for all the average predicted emotions per 6 second emotional expression
total = []
#high contains the highest activated emotions per 6 second emotional expression
...
#this list will contain timestamps for each emotionlly expressed video with respect to 'time'
video = []
#looping from 1 to 7, to add the different timestamps
for i in range(1,7):
    #each video is 6 seconds hence timestamps set as such
...
for i in range(6):
    ...
    #find the average value for each emotions in these 6 seconds observed seconds
    ...
    #a list of the average prediction value in these 6 seconds for each emotion is appended
    videofeeling.append([round(angryP,5), round(fearP,5), round(happyP,5), round(sadP,5), round(surpriseP,5), round(neutralP,5)])
    
    #the containers for all emotions observed for the 6 second are reset
    angry = []
    fear = []
    happy = []
    sad = []
    ...
    #'high' gets the most activated emotions in these 6 seconds in words
    high.append(CATEGORIES[np.where(videofeeling==np.max(videofeeling))[1][0]])
    
    #total gets the list with the average activation value per 6 second expression
...
order = ['sad', 'angry', 'fear', 'neutral', 'surprise', 'happy']
...
for ax in axs.flat:
    ...
    plt.xticks(np.arange(6), order, rotation=45)
